...
TestCase.Assertions = {
  ...
  assert_array_contains: function(arr, value, message) {
    for (var i=0; i < arr.length; i++) {
      if (arr[i] == value) return;
    }
    this.throw_problem("Array does not contain the expected item.", message);
  },
  assert_array_not_contains: function(arr, value, message) {
    for (var i=0; i < arr.length; i++) {
      if (arr[i] == value)
        this.throw_problem("Array contains the restricted item.", message);
    }
  },
